...
TL;DR: 该研究探讨了自闭症谱系障碍儿童在技能发展前的行为模式和情绪识别，通过纵向监测建立基线理解，并提出针对性的技术辅助框架。
...
Method: 采用纵向方法监测情绪和行为，分析随时间变化的行为趋势，建立对自闭症学生独特需求和挑战的基线理解。
...
Method: 1. 模态感知协同进化框架：协调器划分全局染色体为模态特定基因组，本地工作者独立进化，协调器重组染色体进行联合评估；2. 模态感知双轨代理方法减少评估成本；3. 基于相似性的种群多样性指标策略平衡探索与利用。

Result: 在标准漏洞共利用数据集上，MACC-MGNAS在仅3 GPU小时内达到81.67%的F1分数，比最先进方法提高8.7% F1，同时减少27%计算成本。

Conclusion: MACC-MGNAS有效解决了多模态图神经网络架构搜索中的模态异质性挑战，显著提升了漏洞共利用攻击预测性能并降低了计算开销。
...
Conclusion: MultiFair通过双层级梯度调制有效解决了多模态医学分类中的不平衡和公平性问题，在多个数据集上表现出优越性能。
...
TL;DR: 评估两种先进作物产量预测模型(GNN-RNN和MMST-ViT)在真实分布外条件下的泛化能力，发现GNN-RNN在跨区域迁移中表现更优，而模型复杂度不是泛化性能的关键因素。


<details>
  <summary>Details</summary>
Motivation: 气候变化正日益破坏农业系统，需要准确的作物产量预测来保障粮食安全。虽然深度学习模型在利用卫星和天气数据进行产量预测方面显示出潜力，但它们在跨地理区域和年份的泛化能力（对实际部署至关重要）尚未得到充分测试。

Method: 使用覆盖2017-2022年1200多个美国县的大规模CropNet数据集，通过跨七个USDA农场资源区域的留一聚类交叉验证和年度预测场景，对GNN-RNN和MMST-ViT两种模型进行基准测试。

Result: GNN-RNN在跨地理迁移中表现出更好的泛化能力（正相关），而MMST-ViT在域内表现良好但在分布外条件下急剧退化。Heartland和Northern Great Plains等区域显示出稳定的迁移动态，而Prairie Gateway在所有模型和作物中持续表现不佳。GNN-RNN的训练速度比MMST-ViT快135倍。

Conclusion: 时空对齐（而非模型复杂度或数据规模）是稳健泛化的关键，需要透明的分布外评估协议来确保公平可靠的气候感知农业预测。
...
TL;DR: 为了解决GPU内核生成任务中高质量数据稀缺的问题，作者开发了一个生成和筛选高质量CUDA内核及其推理轨迹的流程，构建了ConCuR数据集并训练了KernelCoder模型，在KernelBench上显著超越了现有最佳模型。


<details>
  <summary>Details</summary>
Motivation: GPU内核生成面临高质量数据稀缺的挑战，因为大多数高质量内核是专有且不开源的，这阻碍了使用监督微调将LLM与内核生成任务对齐。

Method: 开发了一个生成和筛选高质量CUDA内核及其推理轨迹的流程，构建ConCuR数据集，训练KernelCoder模型，该模型是首个在包含PyTorch、推理和CUDA内核对的数据集上训练的模型。
...
TL;DR: ETD方法通过在训练阶段让模型在推理相关层上进行迭代，增强基础模型的推理能力，无需改变架构或参数数量，在多个推理基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有提升LLM推理能力的方法主要依赖参数规模扩展或复杂思维链生成，而研究表明推理所需的关键计算集中在有限层中，因此探索更高效的推理增强方法。

Method: 提出Encode-Think-Decode方法，在中期训练阶段训练模型在选定的推理相关层上进行迭代，同时保持原始架构、参数和训练数据不变。
...
### [7] [Best-of-Both Worlds for linear contextual bandits with paid observations](https://arxiv.org/abs/2510.07424)
...
### [8] [Learning to Route LLMs from Bandit Feedback: One Policy, Many Trade-offs](https://arxiv.org/abs/2510.07429)
*Wang Wei,Tiankai Yang,Hongjie Chen,Yue Zhao,Franck Dernoncourt,Ryan A. Rossi,Hoda Eldardiry*

Main category: cs.LG

TL;DR: BaRP是一种基于偏好调整的在线路由方法，通过上下文多臂赌博机框架在训练时模拟部署中的部分反馈条件，支持在测试时无需重新训练即可调整性能/成本权衡


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由方法大多依赖离线训练和所有候选模型的完整标签，这与实际部署中只能观察到所选模型结果的限制不符，导致系统要么为强模型过度付费，要么面临弱模型性能不佳的风险

Method: 将LLM路由问题建模为基于提示特征和用户偏好向量的上下文多臂赌博机问题，在训练时模拟在线反馈设置，使路由决策能够适应每个新提示

Result: 在综合实验中，该方法比强离线路由器性能提升至少12.46%，比最大LLM性能提升至少2.45%，并且在未见任务上具有鲁棒泛化能力

Conclusion: BaRP方法成功弥合了训练和部署之间的反馈差距，提供了一种在部分反馈条件下有效训练LLM路由器的解决方案，支持灵活的性能成本权衡调整
...
### [9] [Parameter-Free Federated TD Learning with Markov Noise in Heterogeneous Environments](https://arxiv.org/abs/2510.07436)
*Ankur Naskar,Gugan Thoppe,Utsav Negi,Vijay Gupta*

Main category: cs.LG

TL;DR: 提出了一种参数无关的联邦时差学习算法，在马尔可夫数据下实现了最优的收敛速率，适用于异构环境的联邦学习场景。


<details>
  <summary>Details</summary>
Motivation: 现有联邦强化学习在马尔可夫数据下的TD学习算法需要依赖未知问题参数才能达到最优收敛速率，这限制了实际应用。

Method: 设计了双时间尺度的联邦时差学习算法，结合Polyak-Ruppert平均技术，无需依赖问题参数。

Result: 算法在平均奖励和折扣设置下均能实现最优的O(1/NT)收敛速率，即使在异构环境中也适用。

Conclusion: 该方法填补了联邦强化学习在马尔可夫数据下的理论空白，为参数无关的联邦TD学习提供了有效解决方案。
...
### [10] [MoGU: Mixture-of-Gaussians with Uncertainty-based Gating for Time Series Forecasting](https://arxiv.org/abs/2510.07459)
*Yoli Shavit,Jacob Goldberger*

Main category: cs.LG

TL;DR: MoGU是一种用于回归任务和时间序列预测的新型混合专家框架，通过将每个专家的输出建模为高斯分布来量化预测的不确定性，并使用基于不确定性的门控机制替代传统的基于输入的门控网络。


<details>
  <summary>Details</summary>
Motivation: 传统混合专家框架只提供点估计，无法量化预测的不确定性，而时间序列预测中准确的不确定性估计对于决策至关重要。

Method: 将每个专家的输出建模为高斯分布，使用专家估计的方差来确定其对最终预测的贡献，替代传统的基于输入的门控网络。

Result: 在多个时间序列预测基准测试中，MoGU始终优于单专家模型和传统混合专家设置，并提供与预测误差直接相关的良好量化不确定性。

Conclusion: MoGU通过基于不确定性的门控机制显著提升了时间序列预测的准确性和可靠性，为回归任务提供了有效的混合专家解决方案。
...
### [11] [FedQS: Optimizing Gradient and Model Aggregation for Semi-Asynchronous Federated Learning](https://arxiv.org/abs/2510.07664)
*Yunbo Li,Jiaping Gui,Zhihang Deng,Fanchao Meng,Yue Wu*

Main category: cs.LG

TL;DR: FedQS是首个在SAFL中理论分析并解决梯度聚合与模型聚合差异的框架，通过将客户端分类为四种类型并自适应优化本地训练，在准确率、收敛速度和稳定性方面均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决半异步联邦学习中梯度聚合和模型聚合策略之间的权衡问题，梯度聚合收敛快但波动大，模型聚合稳定但收敛慢且精度低。

Method: 提出FedQS框架，采用分而治之策略，根据数据分布特征和计算资源将客户端分类为四种类型，并自适应优化其本地训练。

Result: 在计算机视觉、自然语言处理和真实世界任务上的大量实验表明，FedQS实现了最高准确率、最低损失，并且在收敛速度方面排名前列，优于现有最优基线方法。
...
### [12] [metabeta - A fast neural model for Bayesian mixed-effects regression](https://arxiv.org/abs/2510.07473)
*Alex Kipnis,Marcel Binz,Eric Schulz*

Main category: cs.LG

TL;DR: 提出metabeta，一种基于transformer的神经网络模型，用于贝叶斯混合效应回归，相比MCMC方法大幅减少推理时间
...
### [13] [SketchGuard: Scaling Byzantine-Robust Decentralized Federated Learning via Sketch-Based Screening](https://arxiv.org/abs/2510.07922)
*Murtaza Rangwala,Farag Azzedin,Richard O. Sinnott,Rajkumar Buyya*

Main category: cs.LG

TL;DR: SketchGuard是一个去中心化联邦学习的拜占庭鲁棒防御框架，通过草图压缩技术大幅降低通信和计算开销，同时保持与现有方法相同的鲁棒性。
...
Method: 使用Count Sketch将d维模型压缩到k维草图进行相似性比较，仅从接受的邻居处获取完整模型，将每轮通信复杂度从O(d|N_i|)降低到O(k|N_i| + d|S_i|)。

Result: 在多个数据集、网络拓扑和攻击场景下，SketchGuard保持与最先进方法相同的鲁棒性，同时减少计算时间达82%，通信开销减少50-70%。

Conclusion: 基于草图压缩的方法是实现网络规模鲁棒去中心化联邦学习的基本使能技术。
...
### [14] [Surrogate Modeling for the Design of Optimal Lattice Structures using Tensor Completion](https://arxiv.org/abs/2510.07474)
*Shaan Pakala,Aldair E. Gongora,Brian Giera,Evangelos E. Papalexakis*

Main category: cs.LG

TL;DR: 该论文提出使用张量补全作为替代模型，在材料设计搜索空间存在偏差采样的情况下，相比传统机器学习方法（如高斯过程和XGBoost）能获得更好的机械性能预测效果。


<details>
  <summary>Details</summary>
Motivation: 材料设计中，随着设计变量增加，搜索空间呈指数增长，使得合成和验证每个材料的性能变得不切实际。传统机器学习方法在训练数据来自非均匀随机采样时表现不佳，因为实验者可能因便利性而更频繁地合成某些材料。

Method: 采用张量补全作为替代模型来加速材料设计，特别针对搜索空间存在偏差采样的非典型监督学习场景。

Result: 实验表明，在存在偏差采样的搜索空间中，张量补全优于经典机器学习方法（高斯过程和XGBoost），R²提高了约5%。在均匀随机采样下，张量补全仍能提供可比的性能。

Conclusion: 张量补全是在材料设计搜索空间存在偏差采样情况下的有效替代模型，能够显著加速具有特定机械性能的晶格结构设计。
...
### [15] [From Tokens to Layers: Redefining Stall-Free Scheduling for LLM Serving with Layered Prefill](https://arxiv.org/abs/2510.08055)
*Gunjun Lee,Jiwon Kim,Jaiyoung Park,Younjoo Lee,Jung Ho Ahn*

Main category: cs.LG

TL;DR: 提出分层预填充调度方法，通过将模型垂直分区为连续层组，在层组间交错执行预填充和解码，消除MoE模型中的块化权重重载问题，显著降低延迟和能耗。


<details>
  <summary>Details</summary>
Motivation: 现有块化预填充调度方法虽然能稳定TBT，但在MoE模型中会导致大量专家权重重载，增加内存流量和能耗。

Method: 将transformer层组作为主要调度单元，垂直分区模型并在层组间交错执行预填充和解码操作。

Result: 降低TTFT达70%，端到端延迟降低41%，每token能耗降低22%，减少专家加载流量和能耗成本。

Conclusion: 从token维度转向层维度的调度轴解锁了新的高效能、能耗感知的LLM服务操作机制。
...
### [16] [HEMERA: A Human-Explainable Transformer Model for Estimating Lung Cancer Risk using GWAS Data](https://arxiv.org/abs/2510.07477)
...
TL;DR: HEMERA是一个基于可解释Transformer的深度学习框架，直接处理GWAS原始基因型数据预测肺癌风险，无需临床协变量，在27000多名参与者数据上实现了>99%的AUC得分。


<details>
  <summary>Details</summary>
Motivation: 肺癌是美国第三常见癌症和主要癌症死因，虽然吸烟是主要风险因素，但不吸烟者和家族聚集性研究表明存在遗传成分。GWAS识别的遗传生物标志物有望用于评估肺癌风险。

Method: HEMERA框架应用可解释Transformer深度学习处理GWAS的SNP数据，引入加性位置编码、神经基因型嵌入和精炼变异筛选，基于分层集成梯度的后验可解释性模块可将预测归因于特定SNP。

Result: 在27,254名百万退伍军人计划参与者数据上训练，HEMERA实现了>99%的AUC得分，模型预测能强烈对齐已知的肺癌风险位点。
...
### [17] [DYNAMIX: RL-based Adaptive Batch Size Optimization in Distributed Machine Learning Systems](https://arxiv.org/abs/2510.08522)
...
<details>
  <summary>Details</summary>
Motivation: 现有的分布式机器学习批量大小选择方法依赖静态分配或简单启发式，无法适应异构、动态的计算环境，需要更智能的自适应方法。

Method: 采用强化学习框架，使用PPO算法和多维状态表示（网络指标、系统资源利用率、训练统计效率指标），无需显式系统建模即可与现有分布式训练框架集成。

Result: 在多样化工作负载、硬件配置和网络条件下，DYNAMIX实现了最终模型精度提升6.3%，总训练时间减少46%，在32节点集群上保持最佳性能，且学习策略能有效迁移到相关模型架构。

Conclusion: DYNAMIX证明了强化学习在分布式机器学习批量大小优化中的有效性，能够适应异构动态环境，显著提升训练效率和模型质量。
...
TL;DR: 本文提出了一种基于强化学习的任务卸载框架，用于可穿戴物联网设备，通过Q学习算法优化能耗与任务完成时间的权衡。
...
Method: 将任务卸载问题建模为马尔可夫决策过程，使用Q学习技术让可穿戴设备在没有先验知识的情况下做出最优任务卸载决策。

Result: 通过ns-3网络模拟器进行广泛仿真，评估了不同应用和系统配置下的性能，并分析了Q学习主要参数对任务完成时间、能耗和卸载比例的影响。

Conclusion: 强化学习框架能够有效优化可穿戴设备的任务卸载决策，在能耗和延迟之间取得良好平衡。
...
TL;DR: SurpMark是一个基于参考的机器生成文本检测器，通过量化token surprisals的动态变化来检测文本，使用广义Jensen-Shannon差距比较测试文本与人类/机器参考之间的状态转移差异。
...
Method: 将surprisals量化为可解释状态，估计测试文本的状态转移矩阵，通过广义Jensen-Shannon差距比较测试转移与预构建的人类和机器参考之间的差异。

Result: 在多个数据集、源模型和场景下，SurpMark始终匹配或超越基线方法，实验证实了统计量的渐近正态性，消融实验验证了离散化的有效性。

Conclusion: SurpMark提供了一种有效且可解释的机器生成文本检测方法，在实用约束下表现优异。
...
TL;DR: PEAR基准测试系统评估规划器-执行器多智能体系统的效用和脆弱性，发现规划器比执行器更易影响系统性能，存在性能与鲁棒性权衡，针对规划器的攻击特别有效。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常只考察孤立的攻击面或特定场景，缺乏对多智能体系统脆弱性的整体理解，需要系统性的评估基准。

Method: 引入PEAR基准测试，专注于广泛采用的规划器-执行器结构，通过大量实验系统评估多智能体系统的效用和脆弱性。

Result: 发现弱规划器比弱执行器更严重影响系统性能；规划器需要记忆模块而执行器不需要；存在性能与鲁棒性权衡；针对规划器的攻击特别有效。
...
<details>
  <summary>Details</summary>
Motivation: 解决在标记数据有限且存在分布偏移的情况下，如何提升模型泛化能力的问题，为开发数据高效且鲁棒的AI系统提供理论基础。
...
Conclusion: 多模态协同训练作为一种结构化方法，在动态现实环境中开发数据高效且鲁棒的AI系统具有实际优势。
...
TL;DR: MLLM4TS是一个利用多模态大语言模型进行时间序列分析的框架，通过视觉表示和数值数据的融合，在分类、异常检测和预测等任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 受人类分析师视觉检查时间序列的启发，旨在解决多变量时间序列中复杂时间依赖性和跨通道交互的挑战，弥合连续数值数据与离散自然语言之间的模态差距。

Method: 提出MLLM4TS框架，将每个时间序列通道渲染为水平堆叠的彩色线图，采用时间感知视觉块对齐策略，融合数值数据的细粒度时间细节和视觉表示的全局上下文信息。
...
TL;DR: 提出基于小波变换的自动睡眠分期框架，通过连续小波变换生成时频图，结合集成学习实现88.37%的准确率和73.15的宏平均F1分数，优于传统机器学习方法，与深度学习表现相当。


<details>
  <summary>Details</summary>
Motivation: 准确的睡眠分期对睡眠障碍诊断至关重要，传统方法依赖人工标注或时域/频域特征提取，需要更自动化和准确的方法。

Method: 使用连续小波变换生成时频图捕捉睡眠分期的瞬态和振荡模式，结合集成学习方法进行睡眠分期。

Result: 在Sleep-EDF扩展数据库上达到88.37%的总体准确率和73.15的宏平均F1分数，优于传统机器学习方法，与深度学习表现相当或更好。

Conclusion: 小波分析在睡眠分期中具有稳健、可解释和临床应用的潜力。
...
TL;DR: 提出FairSpecTemp方法从图平稳节点观测中估计公平图，确保连接不偏袒敏感属性。该方法有两种变体：一种直接约束偏置，另一种通过限制图谱中的偏置来隐式鼓励公平估计。


<details>
  <summary>Details</summary>
Motivation: 现实世界图中的边往往表现出对某些群体对的连接偏好，这种有偏连接会加剧甚至引发下游基于图的任务的不公平处理。

Method: 提出Fair Spectral Templates (FairSpecTemp)优化方法，有两种变体：一种利用图平稳性的交换性特性直接约束偏置，另一种通过限制图谱中的偏置来隐式鼓励公平估计。

Result: 方法具有高概率性能界限，在公平性和准确性之间产生条件权衡。分析表明恢复公平图不一定需要牺牲准确性。在合成和真实数据集上的评估证明了方法的有效性。

Conclusion: FairSpecTemp能够有效估计公平图，两种变体各有优势，为图数据公平性提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为了在数字孪生中高效预测感兴趣量的长期动态，避免每次都需要运行计算成本高昂的全系统仿真。

Method: 使用基于记忆的流映射学习(FML)，利用全数字孪生生成的短轨迹数据构建数据驱动模型，将tDT构建过程完全离线化。

Result: 在二维不可压缩圆柱绕流CFD示例中，tDT能够准确预测作用在圆柱上的水动力，完全绕过全流场仿真。

Conclusion: 该方法成功构建了紧凑的动态系统，能够高效准确地预测感兴趣量的长期动态，显著节省计算成本。
...
TL;DR: 本文研究了两层神经网络中dropout在大宽度极限下的渐近行为，揭示了五个不同的非退化相，表明dropout的惩罚效应仅在极小学习率下存在，而在较大学习率下等效于随机几何技术。
...
Method: 使用平均场初始化尺度，研究两层神经网络在梯度下降训练中使用dropout的大宽度渐近行为，结合平均场粒子系统和随机过程的工具进行收敛性证明。

Result: 获得了丰富的渐近相图，包含五个不同的非退化相，发现dropout的惩罚效应仅在O(1/宽度)量级的极小学习率下存在，在较大学习率下等效于随机几何技术。

Conclusion: 为重新理解大规模神经网络中dropout的理论基础奠定了基础，揭示了dropout在不同学习率机制下的等效行为。
...
TL;DR: 使用BERTopic对lmsys-chat-1m多语言对话数据集进行主题建模，分析主题模式与用户偏好的关系，识别特定主题下LLM的偏好趋势。


<details>
  <summary>Details</summary>
Motivation: 揭示对话中的主题模式及其与用户偏好的关系，探索特定主题下不同LLM是否被一致偏好。

Method: 设计多语言预处理流程，使用BERTopic提取主题，通过可视化技术分析主题与模型偏好的关系。

Result: 提取了29个连贯主题（如AI、编程、伦理等），分析了主题与模型偏好的关联趋势。
...
Result: 在合成和机器人基准测试中表现出优越性能，证明了EBGAN-MDN在多模态学习任务中的有效性和效率。
...
TL;DR: LOTUS是一个简单有效的无监督机器学习模型选择方法，使用最优传输距离来度量未标记表格数据集之间的相似性，为离群值检测和聚类任务推荐机器学习流程。


<details>
  <summary>Details</summary>
Motivation: 直觉是如果一个机器学习流程在具有相似底层数据分布的数据集上表现良好，那么它在新数据集上也会表现良好。

Method: 使用最优传输距离计算未标记表格数据集之间的相似性，为离群值检测和聚类这两个无监督任务推荐机器学习流程。

Result: 实验表明LOTUS相比强基线方法表现有效，是无监督机器学习任务模型选择的一个有前景的第一步。

Conclusion: LOTUS是面向多个无监督机器学习任务的模型选择的一个有前景的方法。
...
TL;DR: 提出Symbolic Diffusion，一种基于D3PM的离散状态空间扩散模型，用于符号回归任务，能够同时生成所有方程标记，相比自回归模型在某些指标上表现更好。
...
Conclusion: 基于扩散的生成方法为神经网络符号回归开辟了新的研究机会，在相似底层架构下可以媲美甚至超越自回归生成。
...
TL;DR: 本文对液体神经网络(LNNs)与传统循环神经网络(RNNs)及其变体(LSTMs、GRUs)进行了比较分析，重点关注模型准确性、内存效率和泛化能力。研究发现LNN在处理噪声、非平稳数据和实现分布外泛化方面具有显著潜力，且某些LNN变体在参数效率和计算速度上优于传统RNN。


<details>
  <summary>Details</summary>
Motivation: 比较分析新兴的液体神经网络与传统循环神经网络在序列数据处理方面的性能差异，探索LNN作为生物启发的连续时间动态神经网络的潜力。

Method: 通过系统回顾现有研究，分析这些神经网络架构的基本原理、数学模型、关键特征和固有挑战，重点关注模型准确性、内存效率和泛化能力三个核心维度。

Result: LNN在处理噪声和非平稳数据方面表现出色，能够实现分布外泛化，某些LNN变体在参数效率和计算速度上优于传统RNN。但RNN由于其成熟的生态系统和在各种任务中的成功应用，仍然是序列建模的基石。

Conclusion: 本文识别了LNNs和RNNs的共同点和差异，总结了各自的缺点和挑战，并指出了未来研究的有价值方向，特别强调提高LNNs的可扩展性以促进其在更广泛和复杂场景中的应用的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM的环境交互必须通过预定义格式的文本表达，需要手动解析器，这使模型的语言既要负责推理又要负责控制。作者希望将环境交互从语言中解耦，简化交互过程。

Method: 提出ExpA方法，内部化环境交互到扩展动作空间，模型可以在语言环境和外部环境之间切换；引入EARL强化学习算法，使用反事实策略优化来促进对新环境和扩展动作空间的探索。

Result: 在需要多轮交互和条件规划的任务中，EARL优于词汇约束动作的基线方法；在基于计算器的多任务学习中表现稳健；在部分可观测排序问题中达到100%的Sort-4准确率，并自主发现了与经典设计相竞争的高效算法。

Conclusion: ExpA方法成功地将环境交互从语言中解耦，EARL强化学习算法能够有效探索扩展动作空间，在复杂交互任务中展现出优越性能，为LLM与环境交互提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前时序图机器学习缺乏成熟的基础设施，现有库通常针对特定架构设计，且连续时间和离散时间方法之间存在割裂，限制了模型多样性和直接比较。

Method: 开发TGM时序图建模库，统一连续时间和离散时间动态图方法，支持动态节点特征、时间粒度转换，以及链接、节点和图级别的原生任务处理。

Result: TGM在多个模型、数据集和任务上相比广泛使用的DyGLib平均加速7.8倍，在图离散化方面相比现有实现平均加速175倍，并实现了动态图属性预测和时间驱动训练等新研究可能性。

Conclusion: TGM填补了时序图机器学习基础设施的空白，通过统一框架显著提升了效率，并为之前难以研究的问题打开了大门。
...
TL;DR: 本文提出了一种基于注意力机制的Transformer模型，用于铁路轨道间接结构健康监测中的断轨检测，通过无监督学习方法解决小尺寸瞬态异常检测难题，并在合成数据基准测试中验证了模型性能。


<details>
  <summary>Details</summary>
Motivation: 间接结构健康监测(iSHM)使用车载传感器进行铁路轨道评估具有成本效益，但由于复杂车辆动力学、信号噪声和标记数据稀缺，可靠检测小尺寸(2-10厘米)瞬态异常仍面临重大挑战。

Method: 提出注意力聚焦Transformer模型，采用自注意力机制，通过重构训练但创新地从学习到的注意力权重偏差中推导异常分数，旨在实现有效性和计算效率。建立了增量合成数据基准来系统评估模型鲁棒性。

Result: 基准测试结果显示，虽然基于Transformer的模型普遍优于其他模型，但所有测试模型都对高频局部噪声表现出显著脆弱性。提出的模型在达到最先进解决方案相当精度的同时，展现出更好的推理速度。
...
TL;DR: DGTEN提出了一种基于高斯分布的动态信任评估网络，通过不确定性感知的消息传递、时间建模和防御机制，在比特币信任网络中显著提升了信任预测性能。
...
Result: 在比特币信任网络上，单时间槽预测MCC提升10.77%，冷启动场景提升16.41%，对抗攻击下提升11.63%。

Conclusion: DGTEN框架在动态信任评估中表现出色，验证了其统一方法的有效性。
...
TL;DR: 本文对大型语言模型(LLM)的遗忘方法进行了系统分类和评估，提出了更全面的评估指标，揭示了现有评估方法的局限性。
...
Method: 将12种状态遗忘方法分为三类：差异驱动优化、表示错位和基于拒绝的目标遗忘，并引入开放式问答指标来评估生成性能。

Result: 分析显示当前评估高估了成功，忽视了实际生成行为；开放式问答指标更好地揭示了UE-UT权衡；不同攻击场景下的鲁棒性差异显著。

Conclusion: 需要更全面的评估框架来指导LLM遗忘方法的设计和评估，重点关注生成行为和不同攻击场景的鲁棒性。
...
TL;DR: 使用聚类技术对疫情期间活跃的度假租赁房产进行分类，以识别固有模式和行为特征


<details>
  <summary>Details</summary>
Motivation: 理解度假租赁市场的复杂性，识别疫情期间房产和房主的共同特征，为制定针对性政策提供依据

Method: 使用K-means和K-medoids聚类技术，分析超过100万处房产和房主的数据集

Result: 识别出同质化的房产群体及其共同特征

Conclusion: 研究结果增强了对度假租赁评估复杂性的理解，可用于制定针对特定集群的政策
...
<details>
  <summary>Details</summary>
Motivation: 现有MABNI研究主要关注遗憾最小化，但过度强调最优臂会损害次优臂的推断精度，这一权衡在网络干扰场景中更加显著。

Method: 建立了理论帕累托前沿，设计了EXP3-N-CS算法，包含任意时间有效的渐近置信序列。

Result: 首次在对抗性MABNI中刻画了遗憾最小化与推断精度之间的权衡关系。

Conclusion: 提出的理论框架和算法能够有效平衡MABNI中的遗憾最小化与推断精度权衡问题。
...
Method: 在损失函数中引入基于类间分离(ICS)的正则化项，惩罚模型产生远离先前任务数据聚类中心的输出。同时进行了超参数调优以找到正则化项的最佳权重。

Result: 使用标准的5任务Split CIFAR-10基准和ResNet-18架构，证明了ICS在维持初始任务强性能方面的有效性。但结果也突显了长期知识保留的局限性，特别是在任务数量增加时。

Conclusion: 这项工作强调了持续学习固有的复杂性和权衡，并指出了进一步研究的方向。类间分离正则化技术能够有效缓解灾难性遗忘，但在长期知识保留方面仍有改进空间。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法将未来回报分布简化为单一标量值，而分布RL方法利用回报分布提供更强的学习信号。现有方法使用离散分箱或有限分位数估计回报分布，但无法揭示回报分布的细粒度结构，也无法区分具有高回报不确定性的状态用于决策。

Method: 使用现代灵活的基于流模型估计完整未来回报分布；制定新的流匹配目标，生成满足分布贝尔曼方程的概率密度路径；基于学习到的流模型，通过新的流导数ODE估计不同状态的回报不确定性；利用不确定性信息优先学习某些转换上更准确的回报估计。

Result: 在37个基于状态和25个基于图像的基准任务上，Value Flows方法平均成功率提高了1.3倍。

Conclusion: 基于流的模型能够有效估计完整回报分布并识别高不确定性状态，在离线和在线到在线设置中均优于先前方法。
...
TL;DR: HYDRA-EI是一个混合集成增量学习框架，用于预测长期贷款违约，通过多阶段特征处理和模型组合来解决数据分布漂移问题。


<details>
  <summary>Details</summary>
Motivation: 长期贷款违约预测面临借款人行为变化和数据分布随时间漂移的挑战，需要能够适应变化的动态建模方法。

Method: 构建关系型、交叉和频域特征，使用图注意力、自动交叉特征创建和频域变换，采用基于性能的简单方法每周更新模型权重。

Result: HYDRA-EI提高了模型稳定性和泛化能力，适用于长期信用风险任务。

Conclusion: 该框架无需频繁手动调整或固定重训练，能够有效应对长期信用风险评估中的动态变化问题。
...
TL;DR: LiveThinking是一个两阶段优化框架，通过知识蒸馏和强化学习压缩推理路径，解决大推理模型在电商直播中的延迟问题，实现30倍计算成本降低和亚秒级延迟。


<details>
  <summary>Details</summary>
Motivation: AI电商直播中的数字分身需要实时响应来提升用户参与度，但高延迟的大推理模型不适合这种场景。

Method: 第一阶段通过拒绝采样微调将670B教师模型蒸馏到30B MoE模型（3B激活）；第二阶段使用分组相对策略优化的强化学习压缩推理路径，通过多目标奖励函数平衡正确性、帮助性和简洁性。

Result: 计算成本降低30倍，实现亚秒级延迟；在淘宝直播应用中，响应正确性提升3.3%，帮助性提升21.8%；经数十万观众测试，显著提升了商品交易总额。

Conclusion: LiveThinking框架有效提升了实时交互场景中的用户体验和商业表现，证明了在直播环境中优化AI响应的重要性。
...
TL;DR: GRFs++是一种改进的图随机特征方法，通过行走拼接技术和更灵活的行走终止机制，解决了传统GRFs在建模远距离节点关系和采样效率方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统图随机特征(GRFs)存在长期限制，包括难以建模远距离节点关系、依赖采样长随机行走导致效率低下，以及简单的行走终止机制限制了近似精度。

Method: 提出行走拼接技术，将多个短行走拼接而不破坏无偏性；扩展行走终止机制，采用更一般的行走长度分布；用并行计算短行走和矩阵乘法替代顺序采样长行走。

Result: GRFs++继承了长行走的近似质量但效率更高，在不增加计算成本的情况下提高了图核的近似精度，并通过实证评估验证了所有主张。

Conclusion: GRFs++通过创新的行走拼接和灵活的终止策略，显著提升了图随机特征方法的效率和准确性，为图节点上的核计算提供了更优的解决方案。
...
TL;DR: DEAS是一种离线强化学习框架，通过利用动作序列进行价值学习，在复杂长时程任务中表现优异，并能提升大规模视觉-语言-动作模型的性能。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习无需昂贵在线交互即可训练智能体，但现有方法在处理复杂长时程序列决策时仍有困难。

Method: 提出DEAS框架，利用动作序列进行价值学习，通过半马尔可夫决策过程Q学习减少有效规划范围，并采用分离价值学习解决价值高估问题。

Result: DEAS在OGBench的复杂长时程任务中持续优于基线方法，并能显著提升大规模视觉-语言-动作模型在RoboCasa Kitchen仿真和真实世界操作任务中的性能。

Conclusion: DEAS通过动作序列和分离价值学习有效解决了离线强化学习中的长时程规划问题，在复杂任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 由于LBSN签到轨迹数据收集成本高且存在隐私问题，需要生成既能保护隐私又保持真实数据特征的合成数据，但现有方法难以处理轨迹数据的空间离散、时间不规则以及复杂时空模式。

Method: 提出两阶段框架：第一阶段使用稀疏感知的时空扩散模型(S²TDiff)从原始轨迹重建连续规则的运动序列；第二阶段使用Coarse2FineNet（基于Transformer的Seq2Seq架构）生成细粒度轨迹，包含动态上下文融合编码器和多任务混合头解码器。

Result: 在四个真实数据集上的实验表明，GeoGen在保真度和实用性评估上均优于现有最优模型，在FS-TKY数据集上距离和半径指标分别提升69%和55%。

Conclusion: GeoGen框架成功解决了LBSN轨迹生成的挑战，为POI推荐、广告投放和疫情干预等应用提供了高质量的合成数据解决方案。
...
TL;DR: 提出了MeSH方案解决递归Transformer的性能瓶颈，通过外部内存缓冲区和轻量级路由器动态多样化计算，在1.4B规模上超越非递归模型，参数减少33%但准确率提升1.06%。


<details>
  <summary>Details</summary>
Motivation: 递归Transformer虽然参数复用，但在相同计算量下性能落后于非递归模型，主要存在计算未分化和信息过载两个瓶颈。

Method: 引入Memory-as-State-Highways方案，将状态管理外部化到显式内存缓冲区，使用轻量级路由器动态多样化各迭代的计算。

Result: 在Pythia套件上，MeSH增强的递归Transformer持续改进递归基线，在1.4B规模上超越更大的非递归对应模型，平均下游准确率提升1.06%，非嵌入参数减少33%。

Conclusion: MeSH是构建更强递归模型的可扩展和原则性架构。
...
Result: 发现t-SNE会夸大聚类强度，无法可靠显示异常点的极端程度，导致对数据结构的错误解读

Conclusion: t-SNE可视化结果存在固有局限性，不应直接用于推断输入数据的聚类强度和异常点特征
...
TL;DR: FedBook是一个联邦图基础模型代码本，通过两阶段聚合客户端本地代码本，在保护隐私的同时提升跨域图学习性能。


<details>
  <summary>Details</summary>
Motivation: 现有图基础模型需要集中访问多域图数据，但隐私和制度限制使这不可行。联邦图基础模型需要构建既能保证域内语义一致性又能保持跨域多样性的全局代码本。

Method: 采用两阶段过程：1) 域内协作 - 通过参考高频令牌来精炼低频令牌；2) 域间集成 - 根据代码本语义独特性加权聚合客户端贡献。

Result: 在8个基准测试中，FedBook持续优于21个基线方法，包括孤立监督学习、FL/FGL、集中式GFM的联邦适配和FedGFM技术。

Conclusion: FedBook通过系统化的代码本聚合策略，在联邦学习环境下有效解决了图基础模型的跨域泛化问题，同时保护了数据隐私。
...
TL;DR: 提出了一种新的锐度度量方法——Rényi锐度，基于Rényi熵来量化损失Hessian矩阵特征值的分布，并证明了其与泛化能力的强相关性。


<details>
  <summary>Details</summary>
Motivation: 现有锐度度量与泛化能力的相关性通常不强，有时甚至很弱。为了弥合直觉与现实之间的差距，需要一种更有效的锐度度量方法。

Method: 定义Rényi锐度为损失Hessian矩阵的负Rényi熵，利用Rényi熵简洁地表征Hessian特征值的分布程度。还提出了Rényi锐度感知最小化(RSAM)作为正则化方法。

Result: 实验验证了Rényi锐度与泛化能力之间的强相关性（特别是Kendall秩相关）。RSAM方法在所有现有的锐度感知最小化方法中表现最佳，相比经典SAM方法测试准确率提升高达近2.5%。

Conclusion: Rényi锐度是一种有效的泛化能力度量指标，基于Rényi锐度的RSAM训练方法能够显著提升模型性能。
...
Conclusion: VAMO方法能有效提升多任务竞价性能，理论分析验证了收敛性和对齐有效性
...
TL;DR: 提出了一种层间自适应调制方案来减少无线联邦学习中的通信延迟，通过考虑DNN各层的重要性自动决定最优调制级别。


<details>
  <summary>Details</summary>
Motivation: 无线联邦学习中客户端需要通过带宽受限的通道传输高维DNN参数，这导致了通信延迟问题。现有方法对所有DNN层使用相同调制级别，没有考虑各层的重要性差异。

Method: 提出层间自适应调制方案，根据DNN各层的重要性自动决定最优调制级别，为节省延迟提供更多自由度。

Result: 实验结果显示，与现有方案相比，所提方案可节省高达73.9%的通信延迟。

Conclusion: 层间自适应调制方案能有效减少无线联邦学习的通信延迟，通过考虑DNN层间重要性差异实现更高效的通信。
...
TL;DR: 本文扩展了弱形式方程学习方法，结合核密度估计，从稀疏的鳞翅目幼虫种群运动实验数据中学习有效模型，用于预测害虫扩散动态。
...
Method: 扩展弱形式方程学习技术，结合核密度估计，从高度稀疏的实验数据中学习鳞翅目幼虫种群运动的有效模型。使用WSINDy算法等Galerkin方法学习控制方程。

Result: 在模拟农业条件下，对秋粘虫（Spodoptera frugiperda）位置测量的稀疏数据集上展示了该方法的实用性，考虑了不同的植物资源和感染状态。

Conclusion: 弱形式方程学习与核密度估计相结合的方法能够从稀疏数据中有效学习昆虫种群运动模型，为害虫管理提供预测工具。
...
<details>
  <summary>Details</summary>
Motivation: 科学文献中药物动力学信息的提取和标准化是计算药理学中的重大挑战，限制了药物开发中数据驱动模型的可靠性。LLM在文本理解方面取得了显著进展，但在处理生物医学结构化数据时受到异质性、噪声和领域偏移的限制。

Method: 提出了HySim-LLM统一数学计算框架，整合了嵌入加权微调和流形感知去噪方法。建立了两个理论结果：相似性加权泛化边界和基于流形的去噪保证。

Result: 该框架为LLM在结构化生物医学环境中的微调提供了原则性基础，量化了嵌入差异下的适应性能，并限制了噪声或离群样本的损失贡献。

Conclusion: HySim-LLM为生物医学和数据密集型科学领域提供了数学基础可靠的LLM适应路径，增强了模型的可靠性和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法在让LLMs遗忘敏感信息时，往往会损害模型的原有知识和整体效用，需要一种更精细的遗忘方法

Method: SIMU是一个两步框架，通过选择性更新仅负责编码遗忘集的关键神经元来增强二阶优化器基础的遗忘方法
...
Result: 在多个LLM架构上的实验表明，MetaDefense显著优于现有防御机制，对可见和未见攻击模板的有害查询都具有鲁棒防御，同时在良性任务上保持竞争力。
...
TL;DR: 提出了一种测试时自改进方法，通过识别模型不确定的样本、生成类似示例并进行测试时微调，显著提升模型性能，同时大幅减少训练数据需求。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型微调依赖大量训练数据，效率低且成本高，无法保证模型在复杂场景下的泛化能力。现有技术很少评估训练样本是否提供新信息，导致不必要的成本。

Method: 提出三步算法：(1)识别模型难以处理的样本（自感知），(2)从不确定样本生成类似示例（自数据增强），(3)在测试时使用新生成样本进行微调（自改进）。研究了TT-SI（同模型自改进）和TT-D（强模型蒸馏）两种变体。

Result: TT-SI在所有基准测试中平均绝对准确率提升5.48%，超越其他标准学习方法，同时训练样本减少68倍。
...
TL;DR: 提出了一种基于元学习的图级异常检测框架MA-GAD，通过图压缩模块减少噪声干扰，利用元学习从相似网络中提取元异常信息，在少样本条件下显著提升生化数据集上的异常检测性能。
...
Method: MA-GAD框架包含图压缩模块减少图尺寸并保留关键节点信息，采用元学习从相似网络提取元异常信息以学习初始化模型，使用偏置网络增强异常与正常节点的区分度。

Result: 在四个真实生化数据集上的实验表明，MA-GAD在少样本条件下优于现有最先进方法，在图异常和子图异常检测任务上均验证了有效性。

Conclusion: MA-GAD通过结合图压缩和元学习，有效解决了少样本图异常检测中的噪声干扰和标注数据不足问题，在真实数据集上表现出优越性能。
...
TL;DR: 本文综述了扫描电子显微镜(SEM)中信号噪声比(SNR)的优化方法，涵盖噪声来源、测量技术以及从硬件和软件角度提升SNR的策略。


<details>
  <summary>Details</summary>
Motivation: SEM在纳米技术、材料科学和生物成像中至关重要，但噪声会降低图像质量，影响图像的可解释性，因此需要优化SNR。

Method: 回顾SEM成像过程的多个方面，包括SEM的基本操作、噪声来源、SNR测量方法、影响SNR测量的因素，以及从硬件和软件角度提升SNR的方法。

Result: 综述了传统和新兴技术，重点关注它们的应用、优势和局限性，为研究人员和实践者提供了SNR优化的全面理解。

Conclusion: 本文旨在促进该领域的进一步研究，为SEM用户提供SNR优化的综合指南。
...
TL;DR: 提出了一种完整的SEM图像信噪比和噪声方差估计方法，结合机器学习模型指导Wiener滤波器进行噪声去除，构建了更鲁棒的SEM图像滤波流程。


<details>
  <summary>Details</summary>
Motivation: SEM图像常受噪声污染，影响图像质量和后续分析，需要有效的信噪比和噪声方差估计方法来指导噪声去除。

Method: 首先比较五种SNR估计方法，发现LSR方法最优；然后测试SVM和GPR模型与LSR配对，确定Optimizable GPR模型最准确；最终提出AO-GPRLLSR滤波流程，使用估计的噪声方差指导Wiener滤波器。

Result: 提出的AO-GPRLLSR方法在SEM图像SNR和NV估计方面取得显著成功，滤波后获得更低的均方误差。

Conclusion: 该方法为SEM图像噪声去除提供了有效的解决方案，通过准确的噪声参数估计指导滤波过程，显著提升了图像质量。
...
TL;DR: 提出了一个个性化多目标排序系统，包含特征中心、预排序模型、多任务学习模型、多任务融合模块（GRADE框架）和混合排序模型，通过个性化权重融合多种用户反馈信号生成最终排序。


<details>
  <summary>Details</summary>
Motivation: 为了解决多目标排序中不同用户对各项指标的偏好差异，需要开发能够个性化融合多种反馈信号的排序系统。

Method: 使用多任务学习模型预测多种用户反馈信号，然后通过提出的GRADE框架学习个性化权重，将这些权重应用于计算最终得分并生成混合排序。

Result: 构建了一个完整的个性化多目标排序系统架构，能够根据用户个性化偏好融合多种目标生成最终排序结果。
...
TL;DR: 提出了一种基于SNN时间特性的自蒸馏方法，将SNN在时间步上的实例视为子模型，通过强-弱关系进行双向蒸馏，有效提升SNN性能。


<details>
  <summary>Details</summary>
Motivation: 现有SNN性能与ANN存在差距，传统知识蒸馏方法依赖大教师模型或引入额外训练开销，需要更高效的SNN训练方法。

Method: 将SNN在时间步上的实例作为子模型，评估输出置信度识别强弱关系，提出Strong2Weak和Weak2Strong两种自蒸馏方案，支持集成、同步和级联蒸馏实现。
...
TL;DR: 本文提出了四项理论贡献，改进了基于PAC-Bayes边界的神经网络风险证书的可用性，包括：两个伯努利分布KL散度的边界、基于隐式微分的高效方法、非可微目标边界优化，并在MNIST和CIFAR-10上获得首个非平凡泛化边界。


<details>
  <summary>Details</summary>
Motivation: 改进基于PAC-Bayes边界的神经网络风险证书的实用性和紧致性，使其能够实际应用于神经网络训练过程中。

Method: 1) 推导伯努利分布KL散度的紧致边界；2) 基于隐式微分将PAC-Bayesian风险证书优化集成到损失函数中；3) 优化非可微目标（如0-1损失）的边界。

Result: 在MNIST和CIFAR-10数据集上进行了实证评估，首次在CIFAR-10上获得了神经网络非平凡的泛化边界。

Conclusion: 提出的理论贡献显著提高了PAC-Bayes风险证书的实用性和紧致性，为神经网络提供了可计算的非平凡泛化保证。
...
TL;DR: DISCO方法通过选择模型响应差异最大的样本来简化机器学习模型评估，避免了复杂的聚类过程，在多个基准测试中实现了最先进的性能预测结果。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习模型评估成本过高（需要数千GPU小时），这降低了包容性、减缓创新周期并加剧环境影响。现有方法依赖复杂的聚类过程，设计选择敏感。

Method: DISCO方法选择模型响应差异最大的前k个样本，使用贪心的样本级统计而非全局聚类，概念更简单且理论上有信息论最优性。

Result: 在MMLU、Hellaswag、Winogrande和ARC等基准测试中，DISCO方法超越了先前方法，实现了最先进的性能预测结果。

Conclusion: 促进样本多样性不是关键，重要的是选择能最大化模型响应多样性的样本。DISCO方法简单有效，为高成本模型评估提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 单细胞扰动预测中，预测新基因扰动效果时需要考虑模型不确定性和数据不确定性，这对于评估预测可靠性至关重要

Method: 开发了PRESCRIBE框架，采用多元深度证据回归方法，能够同时测量模型不确定性和数据不确定性

Result: PRESCRIBE能够有效估计每个预测的置信度得分，与经验准确性强相关，相比基线方法实现了超过3%的稳定准确率提升

Conclusion: PRESCRIBE框架通过联合估计不确定性，能够过滤不可靠的预测结果，显著提高了单细胞扰动预测的可靠性
...
TL;DR: 提出了一个集成高效气候代理模型的MARL框架，用于学习多气体动态下的区域气候政策，实现100倍训练加速且保持政策保真度


<details>
  <summary>Details</summary>
Motivation: 现有气候政策模型计算昂贵，难以嵌入强化学习，需要能捕捉多种温室气体综合影响的高效模型

Method: 使用在20,000条多气体排放路径上预训练的循环神经网络作为CICERO-SCM气候模型的代理模型，集成到MARL环境循环中

Result: 代理模型达到接近模拟器的精度（RMSE≈0.0004K），单步推理快1000倍，端到端训练加速100倍以上，收敛到相同最优政策

Conclusion: 该方法突破了核心计算瓶颈，可在不牺牲政策保真度的情况下进行大规模多气体动态气候政策实验
...
TL;DR: 多轮通信步骤(MGS)作为去中心化与中心化训练之间的桥梁，通过稳定性分析推导了泛化误差和超额误差的上界，揭示了MGS能指数级减少优化误差但无法完全消除与中心化训练的性能差距。


<details>
  <summary>Details</summary>
Motivation: 去中心化训练虽然通信效率高，但性能通常不如中心化训练。MGS作为简单有效的桥梁方法，其理论有效性和能否完全消除性能差距仍是开放问题。

Method: 使用稳定性分析推导MGS的泛化误差和超额误差上界，分析学习率、数据异质性、节点数量、样本大小和通信拓扑等因素在非凸设置下的影响。

Result: MGS能以指数速率减少优化误差边界，但即使MGS趋于无穷，与中心化mini-batch SGD相比仍存在不可忽略的泛化误差差距。在CIFAR数据集上的实验验证了理论发现。

Conclusion: MGS能显著改善去中心化训练性能，但无法完全消除与中心化训练的性能差距，为去中心化训练提供了重要的理论分析框架。
...
TL;DR: 本文首次揭示了现代LLM剪枝方法的安全漏洞：攻击者可以构建一个表面正常的模型，但在剪枝后会表现出恶意行为，成功率高达95.7%-99.5%。


<details>
  <summary>Details</summary>
Motivation: 虽然模型剪枝在减少LLM内存占用方面效果显著，但其安全影响尚未充分探索。本文旨在研究剪枝方法可能被恶意利用的安全风险。

Method: 攻击者首先计算每个参数被剪枝的概率，然后将恶意行为注入到不太可能被剪枝的参数中，再用可能被剪枝的参数修复模型，从而在未剪枝模型中抵消恶意行为。

Result: 在五个模型上的广泛评估显示，经过vLLM中的任意剪枝方法处理后，模型在多种攻击场景下都表现出强烈的恶意行为（越狱成功率95.7%，良性指令拒绝率98.7%，目标内容注入成功率99.5%）。
...
TL;DR: 基于XGBoost的机器学习框架，用于跨地域电力需求预测，整合历史需求、天气和社会经济变量，通过大规模数据集和时序数据分割策略实现准确可扩展的预测。
...
Method: 使用XGBoost梯度提升算法，整合历史电力需求、天气和社会经济变量，采用时序数据分割策略进行模型训练和评估。
...
TL;DR: 提出通过扩展参数数量并继续训练来回收预训练检查点的方法，在相同额外计算预算下比从头训练获得10.66%的准确率提升


<details>
  <summary>Details</summary>
Motivation: 预训练大型语言模型的计算成本快速增长，许多已训练好的检查点由于工程限制或模型容量有限而未被充分利用，需要高效重用这些'沉没'成本

Method: 提出正交增长方法：深度增长采用层间复制，宽度增长采用专家复制并注入噪声；通过扩展实验确定检查点序列中的最佳增长时机
...
TL;DR: 提出基于嵌套演化集过程的框架加速个性化PageRank计算，通过局部化近似近点迭代求解线性系统，时间复杂度为Õ(R²/ε²)或Õ(m)，其中R为常数，m为边数


<details>
  <summary>Details</summary>
Motivation: 解决现有PPR计算方法的效率问题，特别是当1/ε²远小于图规模时，实现与图规模无关的时间复杂度

Method: 使用嵌套演化集过程框架，在每个阶段采用局部化近似近点迭代求解简化线性系统

Result: 算法仅需求解Õ(1/√α)个线性系统，总体时间复杂度为Õ(R²/(√αε²))，当1/ε²≪m时独立于图规模，验证了现有文献中的开放猜想

Conclusion: 该方法在真实世界图上验证了效率，在早期阶段即表现出显著收敛性，为PPR计算提供了高效解决方案
...
TL;DR: 提出基于隐马尔可夫模型的框架，直接从信道传播序列恢复数据采集轨迹，无需位置校准，在MIMO-OFDM网络中实现室内LOS/NLOS条件下的高精度定位。
...
Method: 采用HMM框架联合建模条件传播模型和用户轨迹演化，分别对MIMO网络中的功率、延迟和角度进行建模，区分LOS/NLOS条件，使用高斯-马尔可夫模型建模用户轨迹。

Result: 在模拟MIMO-OFDM网络中，使用多天线均匀线性阵列配置，在室内LOS/NLOS环境下实现平均0.65米的定位精度，优于传统的KNN、SVM和DNN方法。

Conclusion: 该方法能够有效构建无线电地图，在无需位置校准的情况下实现高精度定位，为无线通信和定位提供了更高效的解决方案。
...
Method: 提出了后门向量(BV)概念，计算后门模型与干净模型权重之差；开发了稀疏后门向量(SBV)方法将多个攻击合并为一个；提出了注入BV减法(IBVS)作为无假设防御机制。
...
TL;DR: 本文研究发现，仅通过增加模型宽度而不需要参数排列，配合适当的softmax温度校准，就能实现线性模式连接(LMC)。作者提出了层间指数加权连接性(LEWC)来解释这一现象。


<details>
  <summary>Details</summary>
Motivation: 先前研究认为实现LMC需要参数排列搜索和足够宽的模型。本文旨在验证仅通过增加模型宽度是否足以实现LMC，并解释其背后的机制。

Method: 使用softmax温度校准方法，分析中间层输出，提出层间指数加权连接性(LEWC)概念，将合并模型的每层输出表示为原始模型对应层输出的指数加权和。

Result: 实证表明，即使没有任何参数排列，仅通过增加模型宽度配合适当的softmax温度校准，就能实现线性模式连接。LEWC分析显示合并模型输出相当于原始模型的集成。

Conclusion: 增加模型宽度不仅促进非线性模式连接，还能显著提高实现线性模式连接的可能性，这为模型合并提供了新的理论解释和实践方法。
...
TL;DR: 提出Subject-Conditioned Layer来解决EEG解码中的主体特定分布偏移问题，通过分解权重为共享组件和个性化低秩修正，提升跨主体模型的性能。


<details>
  <summary>Details</summary>
Motivation: 解决EEG解码中主体特定分布偏移对基础模型开发的障碍，使模型能够适应不同主体的变异性。

Method: 设计自适应层作为标准线性或卷积层的替代，将权重分解为共享的主体不变组件和轻量级的低秩修正，每个主体有独特的修正。
...
TL;DR: 本文研究了学习智能体如何最优地整合专家数据的问题，提出了信息理论框架来量化专家数据的价值，并开发了信息导向的学习策略。


<details>
  <summary>Details</summary>
Motivation: 随着复杂学习智能体越来越多地与现有专家（如人类操作员或已训练智能体）共同部署，需要解决如何最优地整合结构不同的专家数据的问题。

Method: 在贝叶斯多臂老虎机框架下，研究了离线设置（在交互前接收专家最优策略的数据集）和同步设置（在每一步选择基于自身经验还是专家结果更新信念），提出了信息导向规则和信任推断策略。

Result: 证明了在专家数据上预训练可以通过专家数据与最优动作之间的互信息来收紧信息理论遗憾界限，并提出了实用的信息理论算法。
...
Method: 提出了一种新方法，通过显式解耦领域分布并自适应捕捉实例特定的领域信息来解决领域分布高度纠缠的问题。

Result: 大量实验表明，该方法在基于VLM调优技术构建的基线方法上表现更优。

Conclusion: 这项工作为VLMs中实用且细粒度的遗忘铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 现有的GRPO方法存在熵崩溃问题，导致探索消失和策略过早收敛，现有熵正则化方法只能部分缓解问题且引入偏差和不稳定性。

Method: AEPO方法用温度调整分布上的REINFORCE策略梯度替代熵奖励，通过温度调节稳定熵，包含策略梯度正则化、分布正则化和REINFORCE正则化三个关键设计。

Result: AEPO能稳定任意目标水平的熵，有效消除GRPO中的熵崩溃；揭示了熵与性能的非单调关系；提供了更广泛的RFT范式。

Conclusion: AEPO解决了熵控制问题，阐明了熵、探索和推理性能之间的关系，为强化学习微调提供了新的范式。
...
TL;DR: 提出基于香农熵的框架，利用token级对数概率作为置信度信号实现早期停止，在保持任务准确性的同时节省25-50%计算成本。


<details>
  <summary>Details</summary>
Motivation: 利用推理模型中的涌现置信度机制来提升token效率，减少计算开销和延迟。

Method: 使用token级对数概率计算香农熵作为置信度信号，通过熵阈值实现早期停止推理。

Result: 在推理优化模型家族中实现25-50%计算成本节省，同时保持准确性。

Conclusion: 置信度机制是现代后训练推理系统区别于前代模型的关键特征，可通过熵基框架有效利用以实现计算效率提升。
...
TL;DR: GALA是一个可扩展的联邦无监督多源域自适应框架，通过组间差异最小化和温度控制加权策略，有效处理大量异构源域，在多样化多源设置中显著优于现有方法。
...
Conclusion: GALA框架能够稳定并行地训练大量异构源域，为大规模无监督多源域自适应提供了有效的解决方案。
...
TL;DR: 该研究首次比较了商用Wi-Fi在sub-6 GHz和毫米波频段用于人员识别的性能，发现毫米波在低采样率下也能实现高准确率。
...
Method: 使用商用Wi-Fi设备收集同步的sub-6 GHz和毫米波数据，采用相同的训练流程和模型配置，结合背景减除和端到端深度学习。

Result: 在10 Hz低采样率下，毫米波Wi-Fi对20个人的识别准确率达到91.2%。

Conclusion: 毫米波Wi-Fi凭借其更精细的空间分辨率，在人员识别任务中表现出色，即使采样率较低也能获得高准确率。
...
TL;DR: 提出了一种混合框架，通过动态整合非自回归机制的丰富上下文信息来增强自回归生成，在九物种基准测试中显著超越了AR和NAR基线模型。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在生物序列生成中受限于单向性，无法捕捉关键的全局双向标记依赖；非自回归模型虽然提供双向表示，但在生成连贯性和可扩展性方面面临挑战。

Method: 使用共享输入编码器和两个解码器：一个非自回归解码器学习潜在双向生物特征，一个自回归解码器通过新颖的交叉解码器注意力模块整合这些双向特征来合成生物序列。采用重要性退火和交叉解码器梯度阻塞的训练策略。

Result: 在九物种基准测试中，模型显著超越了AR和NAR基线，成功协调了AR的稳定性和NAR的上下文感知能力。

Conclusion: 该研究推进了生物序列建模技术，为增强自回归模型的双向理解提供了新颖的架构范式。
...
<details>
  <summary>Details</summary>
Motivation: 长尾识别在深度学习和基础模型微调中普遍存在且具有挑战性，倾斜的类别分布阻碍模型对尾部类别的泛化能力。现有方法在多标签长尾识别等广泛场景中难以取得一致改进。

Method: 提出模型重平衡(MORE)框架，引入低秩参数组件来调节参数空间分配，采用定制损失和正弦重加权调度，不增加整体模型复杂度或推理成本。

Result: 在多样化的长尾基准测试中，包括多类别和多标签任务，MORE显著提高了泛化能力，特别是对尾部类别，并有效补充现有的不平衡缓解方法。
...
TL;DR: 提出D-SINK框架，通过蒸馏和整合专门处理类别不平衡或标签噪声的'弱'辅助模型的互补见解，增强深度学习模型对长尾噪声数据的双重鲁棒性。
...
Conclusion: 通过协同利用专门处理单一问题的'弱'辅助模型，可以有效解决类别不平衡和标签噪声的联合挑战，D-SINK框架为此提供了有效解决方案。
...
TL;DR: 本文提出并发布了一个新的船舶燃料消耗数据集，建立了标准化基准测试，并首次在船舶燃料消耗建模中应用上下文学习技术，证明了数据驱动燃料预测的可行性。


<details>
  <summary>Details</summary>
Motivation: 航运业中燃料消耗和排放对经济效率和环境可持续性有重大影响，但异构方法和有限的高质量数据集阻碍了建模方法的直接比较。

Method: 引入并发布包含三艘船舶运行和环境数据的新数据集；定义涵盖表格回归和时间序列回归的标准化基准；研究使用TabPFN基础模型在船舶燃料消耗建模中应用上下文学习。

Result: 所有评估模型都表现出强劲性能，支持船上数据驱动燃料预测的可行性。包含环境条件的模型始终优于仅依赖船速的简单多项式基线。TabPFN略优于其他技术，包含时间上下文可提高准确性。

Conclusion: 基础模型与上下文学习能力在表格预测方面具有潜力，包含时间上下文可提高预测准确性，支持数据驱动的船舶燃料消耗预测方法。
...
TL;DR: EVOR是一种可扩展的离线强化学习方法，通过流匹配学习正则化Q函数，在推理时通过拒绝采样进行策略提取，避免了蒸馏或BPTT的计算负担。


<details>
  <summary>Details</summary>
Motivation: 离线RL在机器人领域应用受限，主要因为缺乏可扩展性。现有方法依赖BPTT（计算昂贵）或策略蒸馏（引入复合误差），需要开发不依赖这些方法的可扩展离线RL方法。

Method: EVOR结合表达性策略和表达性价值函数，通过流匹配训练学习最优正则化Q函数，在推理时通过拒绝采样进行策略提取，支持高效优化、正则化和计算可扩展搜索。

Result: 实验表明EVOR在多样化离线RL任务上优于基线方法，证明了表达性价值学习在离线RL中的优势。

Conclusion: EVOR通过整合表达性价值学习，为离线RL提供了一种可扩展的解决方案，避免了传统方法的计算负担和误差问题。
...
TL;DR: 提出了一种轻量级方法PSCBMs，通过在预训练的概念瓶颈模型上添加小型协方差预测模块来建模概念间依赖关系，无需重新训练主干模型，显著提升了干预性能。
...
Result: PSCBMs在测试时持续匹配或优于标准CBMs的概念和目标准确率，在干预下表现显著更好，且比从头训练类似随机模型更高效。

Conclusion: PSCBMs是一种高效的方法，能够在不重新训练主干模型的情况下显著提升概念瓶颈模型的干预性能。
...
TL;DR: 提出了不确定性感知马尔可夫决策过程（UAMDP），将贝叶斯预测、后验采样强化学习和条件风险价值约束规划相结合，用于高风险环境中的序列决策。


<details>
  <summary>Details</summary>
Motivation: 在波动性高、风险大的环境中进行序列决策不仅需要最大化期望收益，更需要原则性的不确定性管理。

Method: UAMDP框架结合了贝叶斯预测、后验采样强化学习（Thompson采样）和条件风险价值（CVaR）约束规划，形成闭环决策系统。

Result: 在高频股票交易和零售库存控制两个领域，UAMDP相比强深度学习基线，长期预测准确率显著提升（RMSE降低25%，sMAPE降低32%），交易夏普比率从1.54提升至1.74，最大回撤减少约一半。

Conclusion: 整合校准的概率建模、与后验不确定性对齐的探索以及风险感知控制，能够产生稳健、可泛化的方法，实现更安全、更有利可图的序列决策。
...
<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)相比自回归大语言模型(AR-LLMs)具有更高的推理吞吐量潜力，但需要专门的强化学习算法来达到可比的推理性能。

Method: 提出了分布匹配策略优化(DMPO)，通过交叉熵优化将dLLM策略分布与最优奖励倾斜分布匹配，并针对小批量训练挑战提出了权重基线减法技术。

Result: 在多个推理基准测试中，DMPO无需监督微调即取得显著提升，准确率比之前SOTA基线提高42.9%，比基础模型提高55.8%。

Conclusion: DMPO证明了分布匹配框架的有效性，为扩散大语言模型的强化学习微调提供了理论基础和实践方案。
...
TL;DR: 该研究使用政治指南针测试评估了8个主流大语言模型的政治偏见和刻板印象传播，发现所有模型都表现出左倾政治倾向，且通过语言变化引发的隐性刻板印象比显性刻板印象更明显。
...
Method: 采用二维政治指南针测试评估模型固有政治倾向，使用角色提示探索显性刻板印象，并通过多语言版本测试揭示隐性刻板印象。

Result: 所有被调查模型都表现出一致的左倾政治倾向；不同模型的刻板印象性质和程度差异很大；通过语言变化引发的隐性刻板印象比显性刻板印象更明显；大多数模型的隐性和显性刻板印象显示出显著一致性。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM作为自主智能体在现实世界环境中部署的增加，多智能体互动变得不可避免。需要理解这类系统中的战略行为，特别是LLM智能体是否能够像强化学习智能体一样，仅通过互动来塑造学习动态并影响其他智能体的行为。

Method: 提出了ShapeLLM方法，这是对无模型对手塑造方法的适配，专门针对基于transformer的智能体。现有对手塑造算法无法直接应用于LLM，因为它们需要高阶导数、面临可扩展性约束或依赖transformer中不存在的架构组件。

Result: 研究表明LLM智能体能够成功地在竞争性游戏（迭代囚徒困境、匹配便士和胆小鬼游戏）中引导对手走向可被利用的均衡，并在合作性游戏（迭代猎鹿游戏和合作版囚徒困境）中促进协调并提高集体福利。

Conclusion: LLM智能体能够通过互动被塑造和塑造他人，确立了对手塑造作为多智能体LLM研究的一个关键维度。
...
Conclusion: Mix-和MoE-DPO通过混合模型的通用函数逼近能力、专家组件的奖励和策略专门化以及上下文相关的软门控，显著提升了DPO在多任务偏好对齐中的性能。
...
TL;DR: 本文解决了高维多元结果的反事实识别问题，使用连续时间流方法建立了反事实识别的基础，确保因果推断的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有反事实推断方法缺乏识别性，削弱了其因果有效性。Pearl认为反事实必须可识别才能证明因果主张。

Method: 使用连续时间流方法，包括非马尔可夫设置，通过动态最优传输工具构建唯一、单调且保序的反事实传输映射。

Result: 在具有反事实真实值的受控场景中验证了理论，并在真实图像上展示了反事实公理合理性的改进。

Conclusion: 建立了多元反事实识别的理论基础，通过流匹配方法确保一致的反事实推断，提高了因果推断的可靠性。
...
Method: 结合傅里叶神经算子(FNOs)学习物理算子和条件流匹配(CFM)进行概率生成，通过时间依赖的分层约束和FNO引导的修正实现物理一致性。

Result: 在谐振子、人类活动识别和锂离子电池退化实验中，相比基线方法生成质量提高16.3%，物理违规减少46%，预测准确率提升18.5%。

Conclusion: 该框架成功将物理先验知识嵌入生成模型，显著提高了生成质量和物理一致性，为物理约束的时间序列生成提供了新范式。
...
TL;DR: 本文提出使用自适应随机森林(ARFs)和漂移感知特征重要性(DAFI)方法来构建适应6G网络动态环境的AI框架，能够处理特征变化并提高预测稳定性，同时通过智能特征重要性计算提升效率。


<details>
  <summary>Details</summary>
Motivation: 6G网络中AI模型需要适应不断变化的环境条件，包括新特征的引入、多供应商部署、硬件升级和服务需求演进，因此需要灵活学习非平稳环境的方法。

Method: 采用自适应随机森林(ARFs)进行迭代训练以实现动态特征适应，并提出漂移感知特征重要性(DAFI)方法，使用分布漂移检测器来决定何时应用计算密集的特征重要性方法。

Result: 测试表明ARFs能够实现稳定预测，随着特征增加精度不断提高；DAFI方法在3个不同数据集上将运行时间减少最多2倍，同时产生更一致的特征重要性值。
...
TL;DR: 提出了RPEL（鲁棒拉取式流行病学习），一种无需中央服务器、可扩展的协作学习方法，通过拉取式流行病通信策略在对抗环境中确保鲁棒学习，通信成本从O(n²)降低到O(n log n)。


<details>
  <summary>Details</summary>
Motivation: 解决协作机器学习中的训练时对抗行为问题，现有方法要么依赖中央服务器，要么通信成本过高，需要一种既鲁棒又高效的分布式学习方法。

Method: 使用拉取式流行病通信策略，节点从小型随机节点子集拉取模型参数，显著减少消息数量，同时保持收敛保证。

Result: 实证结果显示RPEL在对抗环境中保持鲁棒性，与全对全通信的准确性相当，并在大规模网络中高效扩展。

Conclusion: RPEL提供了一种无需中央服务器、通信效率高的协作学习解决方案，在对抗环境中具有鲁棒性和可扩展性。
...
TL;DR: 提出了Learning to Ask (LtA)框架，通过两阶段架构（标准ML模型和增强模型）来优化何时以及如何整合专家输入，比传统的Learning to Defer方法更灵活有效。


<details>
  <summary>Details</summary>
Motivation: 传统Learning to Defer方法将人类和ML模型视为互斥决策者，限制了专家的贡献仅为预测，需要更灵活的人类-AI协作框架。

Method: 采用两阶段架构：标准ML模型和增强模型（训练时加入专家反馈），提供顺序训练和联合训练两种实现方式，设计了具有可实现一致性保证的代理损失函数。

Result: 在合成和真实专家数据上的实验表明，LtA为有效的人类-AI协作提供了更灵活和强大的基础。

Conclusion: LtA框架能够更好地处理何时以及如何整合专家输入，是比传统方法更有效的人类-AI协作解决方案。
...
TL;DR: 该论文研究了transformer在集合补集任务中的长度泛化能力，证明了单层注意力transformer的维度边界，并发现平衡logit位移可促进泛化。研究揭示了softmax压缩和训练噪声是泛化的主要障碍，提出dropout和EMA可分别缓解这些问题。


<details>
  <summary>Details</summary>
Motivation: 研究transformer在集合补集任务中的长度泛化能力，该能力对棋盘游戏式推理至关重要。旨在理解transformer如何处理未见过的序列长度，并识别泛化的关键机制和障碍。

Method: 理论分析单层注意力transformer的维度边界；证明平衡logit位移与长度泛化的关系；通过随机超参数搜索验证dropout和EMA对泛化的作用；在OthelloGPT模型上测试EMA效果。

Result: 建立了transformer维度边界的紧致界；发现平衡logit位移可确保长度泛化；验证了dropout可缓解softmax压缩效应，EMA可减少训练噪声；在OthelloGPT中EMA再次改善了长度泛化。

Conclusion: transformer的长度泛化受softmax压缩和训练噪声限制，dropout和EMA是有效的缓解策略。这些机制在简单和复杂任务中都表现出有效性，为改进transformer的泛化能力提供了理论指导。
...
TL;DR: DeepEN是一个基于深度强化学习的肠内营养个性化推荐框架，在ICU重症患者中提供4小时一次的热量、蛋白质和液体摄入建议，相比临床指南和医生策略能降低死亡率3.7个百分点。


<details>
  <summary>Details</summary>
Motivation: 传统肠内营养治疗主要依赖指南或经验性方法，缺乏对患者个体化生理状态变化的动态响应，需要开发能够根据患者实时生理状态进行个性化营养推荐的数据驱动方法。

Method: 使用MIMIC-IV数据库中11,000多名ICU患者的离线数据，构建包含临床信息的状态空间和定制奖励函数，采用带保守Q学习正则化的双决斗深度Q网络，学习与高价值临床行为一致且避免不安全偏离的策略。

Result: DeepEN在各种定性和定量指标上均优于临床医生和基于指南的策略，估计死亡率从22.5%降至18.8%，关键营养生物标志物也有所改善。
...
TL;DR: 提出一种新的采样算法，通过轻量级微调单层网络，显著提升预训练掩码扩散模型的采样质量和效率，特别在低步数生成场景下表现优异。
...
Method: 采用星形范式重新构建生成过程，引入可学习的重掩码调度器来智能识别和修正可能的错误。

Result: 在文本和代码生成任务上的综合实验表明，该采样算法优于或匹配现有方法，尤其在少量采样步数下带来显著质量提升。

Conclusion: 该方法通过可逆的星形采样范式和智能重掩码机制，有效解决了掩码扩散模型在低步数生成时的性能限制。
...
TL;DR: 评估四种对比学习框架在边缘/雾计算环境中的资源消耗，发现SimCLR在能耗方面表现最佳，并分析了轻量级神经网络架构与CL框架的配对效果。


<details>
  <summary>Details</summary>
Motivation: 对比学习在自监督表示学习中表现出潜力，但在资源受限设备上的部署研究不足。传统CL框架的高计算需求带来了能耗、数据可用性和内存使用方面的挑战。

Method: 系统评估四种广泛使用的CL框架（SimCLR、MoCo、SimSiam和Barlow Twins），采用包括能耗分析和减少训练数据条件的基准测试策略，并评估轻量级神经网络架构与CL框架的配对效果。

Result: 研究发现SimCLR尽管被认为计算成本高，但在各种数据条件下表现出最低的能耗。

Conclusion: 该研究为在资源受限的边缘/雾环境中部署对比学习提供了资源影响的见解，并为其未来优化开辟了多个研究方向。
...
TL;DR: 该论文提出了广义Natarajan维度来表征有限标签多类设置中可原谅0-1损失函数的学习能力。


<details>
  <summary>Details</summary>
Motivation: 研究在有限标签多类设置中可原谅0-1损失函数的学习能力表征问题。

Method: 基于Natarajan维度创建新的组合维度——广义Natarajan维度，并建立与集合值反馈学习的联系。

Result: 证明假设类在该设置下可学习的充要条件是广义Natarajan维度有限，且集合学习问题的可学习性由Natarajan维度表征。

Conclusion: 广义Natarajan维度成功地表征了可原谅0-1损失函数在有限标签多类设置中的学习能力。
...
TL;DR: FlyLoRA是一种基于果蝇嗅觉回路的隐式MoE-LoRA变体，通过秩向专家激活和隐式路由器解决参数干扰问题，在多个任务领域实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统LoRA方法存在参数干扰问题，导致性能不佳。MoE-based LoRA变体虽然能缓解单任务内的相关性，但引入了额外路由器参数，在多任务模型合并中仍存在任务间干扰。

Method: 提出FlyLoRA方法：(1)在上投影矩阵中引入秩向专家激活；(2)使用隐式路由器统一专家路由和下投影，用冻结的稀疏随机投影矩阵替代传统的可训练密集版本。

Result: 在四个领域（通用知识理解、科学问答、数学推理和代码生成）的广泛实验显示，FlyLoRA相比现有方法实现了持续的性能提升。

Conclusion: FlyLoRA不仅取得了实证收益，还展示了生物结构如何启发AI技术创新。该方法消除了显式路由器的需求，同时由于随机矩阵的正交性特性，固有地缓解了任务间干扰。
...
TL;DR: 本研究测试了多种深度学习超分辨率模型，用于提高牙本质孔隙成像的分辨率，发现传统图像质量评估指标与视觉感知不一致，提出了基于生物学结构的评估方法。


<details>
  <summary>Details</summary>
Motivation: 牙本质的力学感知系统依赖于流体通过孔隙网络的流动，但现有共聚焦显微镜的高分辨率成像视野有限，需要更高效的成像方法。

Method: 使用四种深度学习超分辨率模型（RCAN、pix2pix、FSRCNN、CycleGAN）处理配对的低分辨率和高分辨率共聚焦图像，通过像素尺寸增加2倍、4倍、8倍。

Result: 传统图像质量评估指标与视觉感知矛盾，基于孔隙网络结构和连通性的生物学驱动评估能更好地解释超分辨率性能差异。

Conclusion: 针对特定生物结构的评估方法比通用图像质量指标更有效，揭示了模型对弱强度特征的敏感性和图像生成非线性的影响。
...
TL;DR: 论文提出了基于困惑度正则化的提示优化泛化边界分析，在数据稀缺情况下仍能保持非空泛性，并通过实验验证了困惑度正则化对提示泛化的实际益处。


<details>
  <summary>Details</summary>
Motivation: 现有提示工程技术的成功实践与理论分析存在差距，特别是在数据稀缺场景下现有泛化边界变得空泛，需要更全面地解释提示优化的成功机制。

Method: 通过引入数据或分布相关的困惑度作为有效先验，推导新的泛化边界，并分析困惑度正则化如何通过限制探索空间来收紧边界。

Result: 提出的泛化边界在数据稀缺情况下仍保持非空泛性，实验证明困惑度正则化能有效改善提示的泛化性能。

Conclusion: 困惑度作为自然语言任务的先验知识，在提示优化中起到关键作用，能够解释提示工程在实践中的广泛成功，并为数据稀缺场景下的提示优化提供理论支持。
...
TL;DR: 提出DGPO算法，直接利用组内样本的相对偏好信息进行强化学习，无需依赖低效的随机策略，可使用高效的确定性ODE采样器，训练速度提升20倍且性能更优。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO等方法需要随机策略，但最有效的扩散模型采样器基于确定性ODE。使用SDE采样器引入随机性会导致收敛缓慢，需要解决这一矛盾。

Method: 提出Direct Group Preference Optimization (DGPO)，一种在线RL算法，完全摆脱策略梯度框架，直接从组级偏好中学习，利用组内样本的相对信息。

Result: DGPO训练速度比现有最优方法快约20倍，在领域内和领域外奖励指标上均取得更优性能。

Conclusion: DGPO通过直接学习组级偏好，避免了低效随机策略的需求，实现了高效训练和优越性能，为扩散模型的强化学习优化提供了新思路。
...
TL;DR: ClauseLens是一个基于条款的强化学习框架，用于生成透明、合规且风险感知的再保险条约报价，显著减少违规并提高解释准确性。
...
Result: 在多智能体条约模拟器中，ClauseLens将偿付能力违规减少51%，尾部风险表现提高27.9%(CVaR_0.10)，条款基础解释准确率达到88.2%，检索精度87.4%，召回率91.1%。
...
TL;DR: xRouter是一个基于工具调用的路由系统，使用强化学习训练智能路由器，在成本-性能权衡下自动选择直接回答或调用外部模型，无需人工设计路由规则。


<details>
  <summary>Details</summary>
Motivation: 现代LLM部署面临成本与性能的权衡：高端模型性能强但昂贵，轻量级模型经济但复杂任务表现脆弱。静态升级规则和关键词启发式方法无法充分利用这一频谱，也无法跨任务类型自适应。

Method: 使用端到端强化学习训练路由器，采用显式的成本感知奖励函数来编码成本-性能权衡。系统实现包括完整的强化学习框架、奖励和成本核算，以及部署和评估流水线。

Result: 在多样化基准测试中，xRouter实现了强大的成本-性能权衡（例如在可比较的任务完成率下显著降低成本），并为学习路由提供了实证见解。

Conclusion: 这些发现和开源实现为推进学习型、成本感知的LLM编排提供了实用基础，揭示了从模型可训练性到在小规模开源模型中引发复杂编排行为的挑战。
...
TL;DR: 提出了SymTime，一种利用符号信息增强时间序列表示的预训练基础模型，通过系列-符号数据生成机制解决训练数据稀缺问题，在五大时间序列分析任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 时间序列分析基础模型面临训练数据稀缺和不平衡的挑战，需要新的数据生成和预训练机制来克服这些限制。

Method: 基于复杂动态系统理论设计系列-符号数据生成机制，创建高质量时间序列数据与对应符号表达式对，开发SymTime预训练基础模型利用强相关的系列-符号数据对。
...
TL;DR: 本文重新审视了图神经网络中的过度压缩问题，从模型存储和检索能力的角度分析，并提出了一种新的GNN架构来提升信息容量。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在信息传递过程中存在过度压缩问题，即大感受野的节点信息被压缩到固定大小的向量中，造成信息瓶颈。

Method: 通过分析现有测量方法的局限性，引入新的合成任务来验证信息瓶颈问题，并借鉴序列建模中的关联记忆、快速权重编程和xLSTM思想，开发了新型GNN架构。

Result: 新架构在容量合成任务和多个真实图基准测试中表现出色。

Conclusion: 从存储和检索能力角度理解过度压缩问题，并提出了有效的解决方案，提升了GNN的信息处理能力。
...
TL;DR: 提出了一个九维积分签名框架来严格分类激活函数，结合高斯传播统计、渐近斜率和正则性度量，建立了良好定义性、仿射重参数化定律和有界斜率变化下的闭包性质。


<details>
  <summary>Details</summary>
Motivation: 现有激活函数比较大多基于启发式方法，缺乏严格的分类框架来系统分析其表达能力、稳定性和动态特性。

Method: 通过九维积分签名S_sigma(phi)对激活函数进行分类，包括高斯传播统计(m1,g1,g2,m2,eta)、渐近斜率(alpha_plus,alpha_minus)和正则性度量(TV(phi'),C(phi))。

Result: 建立了Lyapunov定理和方差稳定性区域，推导了维度无关的Hessian界，将光滑性与phi'的有界变差联系起来，并对八种标准激活函数进行了分类。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频摘要方法忽略了任务固有的主观性，将多个评分者的评分平均化，无法反映不同人类视角对好摘要的多样化理解。

Method: 采用扩散模型框架，将视频摘要作为条件生成任务，模型学习好摘要的分布并生成多个候选摘要，动态适应视觉上下文。

Result: 在多个基准测试中达到最先进性能，生成的摘要更符合个体标注者偏好，并通过背包问题分析提供新的评估指标。

Conclusion: SummDiff通过条件生成方法有效解决了视频摘要的主观性问题，扩散模型框架能够生成多样化且符合人类偏好的摘要。
...
TL;DR: 提出了In-Context Clustering (ICC)，一种基于LLM的灵活聚类方法，能够处理不同分布的数据，通过注意力机制捕获复杂关系，并在文本编码的数值数据上展示零样本聚类能力。


<details>
  <summary>Details</summary>
Motivation: 传统聚类算法受限于预定义的相似性度量，无法灵活捕捉复杂关系，而LLMs的注意力机制和上下文学习能力为聚类提供了新的可能性。

Method: 使用预训练LLMs的注意力矩阵进行谱聚类，并通过NTP损失对数值和图像数据进行微调，支持文本条件图像聚类。

Result: 在文本编码的数值数据上展示了强大的零样本聚类能力，注意力矩阵显示出明显的聚类模式，性能与传统方法相当。

Conclusion: 将上下文学习扩展到无监督设置，展示了LLMs在聚类任务中的有效性和灵活性，为传统聚类方法提供了新的替代方案。
...
Method: 提出UML（Unpaired Multimodal Learner），采用模态无关的训练范式，单个模型交替处理不同模态输入并共享参数，利用不同模态是共享现实投影的假设。
...
TL;DR: 提出了一个理论框架，通过熵正则化和温度解耦策略，保证收敛到特定最优策略，实现可解释的、保持多样性的最优策略。
...
Method: 使用熵正则化和温度解耦策略，在正则化温度趋近于零时实现可解释的、保持多样性的最优策略。

Result: 该方法能保证收敛到特定最优策略，实现可解释的、保持多样性的策略，并能准确估计相关回报分布。

Conclusion: 提出的框架为策略优化提供了理论保证，实现了可解释性和多样性，并能准确估计回报分布。
...
TL;DR: 本文为RLVR（带可验证奖励的强化学习）建立了理论框架，通过分析梯度间隙和步长阈值，解释了为什么RLVR能够有效工作以及如何避免性能崩溃。
...
Method: 在完整响应（轨迹）和令牌级别分析RLVR训练过程，引入梯度间隙概念来形式化从低奖励到高奖励区域的改进方向，并推导出基于梯度间隙大小的步长阈值。

Result: 证明收敛性关键取决于更新方向与梯度间隙的对齐，发现存在一个尖锐的步长阈值：低于该阈值时学习收敛，高于该阈值时性能崩溃。理论还预测了临界步长如何随响应长度和成功率缩放。

Conclusion: 理论解释了为什么长度归一化等实用启发式方法能提高稳定性，并表明在固定学习率下，成功率可能严格低于100%。通过受控老虎机模拟和LLM实验验证了这些预测。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决模型输出熵控制问题，探索通过输出激活函数来设计更简单、更鲁棒的算法。
...
Conclusion: 输出激活是熵控制的有力工具，为设计更简单、更鲁棒的算法开辟了新方向。
...
TL;DR: 提出了GDPO算法，通过半确定性蒙特卡洛方案降低ELBO估计器的方差，有效解决了扩散语言模型强化学习微调中的似然估计难题。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型具有并行生成和迭代优化的优势，但难以进行强化学习微调，因为无法直接计算似然。现有方法如diffu-GRPO存在严重偏差，而基于ELBO的方法因计算成本过高而应用受限。

Method: 重新审视ELBO估计，分析其方差来源，提出通过快速确定性积分近似降低方差。开发了Group Diffusion Policy Optimization (GDPO)算法，利用半确定性蒙特卡洛方案缓解双蒙特卡洛采样下的方差爆炸问题。

Result: GDPO在数学、推理和编程基准测试中，相比预训练模型有稳定提升，并在大多数任务上优于当前最先进的diffu-GRPO基线方法。

Conclusion: GDPO为扩散语言模型的强化学习微调提供了有效的低方差ELBO估计器，解决了现有方法的偏差和计算成本问题。
...
Method: 提出Linearizer架构：f(x)=g_y^{-1}(A g_x(x))，其中A是线性算子，g_x和g_y是可逆神经网络。通过构造新的加法和标量乘法运算来定义向量空间。

Result: 成功将SVD、伪逆、正交投影等线性代数工具应用于非线性映射；证明共享神经网络的Linearizer组合仍是Linearizer；在扩散模型中实现数百步采样缩减为单步；构建全局投影生成模型和模块化风格迁移。

Conclusion: Linearizer框架为非线性函数提供了线性表示，使线性代数工具可直接应用于神经网络，在采样效率、生成模型和风格迁移等方面展现出显著优势。
...
### [120] [Adaptive Execution Scheduler for DataDios SmartDiff](https://arxiv.org/abs/2510.07811)
*Aryan Poduri*
...
<details>
  <summary>Details</summary>
Motivation: 现有的调度方法在内存使用和延迟优化方面存在不足，需要一种能够智能管理资源分配以避免内存溢出同时优化性能的解决方案。

Method: 使用轻量级预分析器估计字节/行和I/O速率，在线成本/内存模型修剪不安全操作，采用带背压和慢任务缓解的防护爬山策略，根据保守工作集估计选择后端执行模式。

Result: 在合成和公开表格基准测试中，调度器将p95延迟降低23-28%（相比调优预热启发式方法），降低35-40%（相比固定网格基线），同时将峰值内存降低16-22%（相比固定方法降低25-32%），零内存溢出且吞吐量相当。

Conclusion: 该自适应调度器在保持高吞吐量的同时，显著降低了延迟和内存使用，证明了其在资源受限环境下的有效性。
...
### [121] [A Multi-Simulation Bridge for IoT Digital Twins](https://arxiv.org/abs/2510.08164)
...
TL;DR: 提出了DT模拟桥接框架，支持数字孪生与模拟环境之间的双向交互，增强设计灵活性和实时操作能力。


<details>
  <summary>Details</summary>
Motivation: 随着数字孪生在物联网和工业物联网中能力增强，需要与模拟平台无缝集成以支持系统设计、验证和实时操作。

Method: 设计了DT模拟桥接框架，包含架构设计和核心软件组件，支持双向数据交换和灵活互操作性。

Result: 实验结果显示该框架提高了设计敏捷性，促进了虚拟调试，并支持在真实条件下的实时行为分析。

Conclusion: DT模拟桥接框架在多种工业场景中证明了其有效性，能够动态更新数字孪生模型并适应模拟参数。
...
### [122] [Towards Energy-Efficient Serverless Computing with Hardware Isolation](https://arxiv.org/abs/2510.08180)
*Natalie Carl,Tobias Pfandzelter,David Bermbach*

Main category: cs.DC

TL;DR: 论文提出重新设计无服务器计算的硬件架构，使用硬件隔离而非软件隔离，为每个函数分配独立处理器，从而显著降低能耗。


<details>
  <summary>Details</summary>
Motivation: 当前无服务器平台在传统服务器硬件上运行数千个函数，需要昂贵的软件隔离机制和大量空闲服务器来应对负载峰值，导致能效低下。

Method: 采用硬件隔离方法，为每个函数分配独立处理器，构建仅在实际工作时消耗能源的无服务器硬件堆栈。

Result: 初步评估显示，该方法可减少90.63%的能耗开销，平均节省70.8MW。
...
### [123] [Distributed Resource Selection for Self-Organising Cloud-Edge Systems](https://arxiv.org/abs/2510.08228)
*Quentin Renau,Amjad Ullah,Emma Hart*

Main category: cs.DC

TL;DR: 提出了一种分布式资源选择机制，用于云边环境中的动态资源分配，通过分布式决策避免中心化协调瓶颈，实现高效、可扩展和弹性的资源管理。


<details>
  <summary>Details</summary>
Motivation: 在高度动态的云边环境中，集中式协调成为性能瓶颈，需要分布式决策机制来满足复杂分布式应用的资源需求。

Method: 采用基于共识的机制，利用本地知识和代理间协作，不依赖中央控制器，实现分布式编排系统的核心组件。

Result: 计算时间是影响分配决策的关键因素，该方法在不牺牲最优性的情况下实现快速分配，比集中式启发式方法快达30倍。

Conclusion: 该分布式资源选择机制为分布式编排铺平了道路，在大规模场景下能够及时获得结果，而穷举搜索不可行。
...
### [124] [Energy-Efficient Maximal Independent Sets in Radio Networks](https://arxiv.org/abs/2510.08244)
*Dominick Banasik,Varsha Dani,Fabien Dufoulon,Aayush Gupta,Thomas P. Hayes,Gopal Pandurangan*

Main category: cs.DC

TL;DR: 本文提出了无线网络中更节能的最大独立集(MIS)分布式算法，在碰撞检测(CD)和无碰撞检测(no-CD)两种模型下分别实现了O(log n)和O(log²n log log n)的能量复杂度，并证明了CD模型下能量复杂度的最优性。


<details>
  <summary>Details</summary>
Motivation: 无线网络通常由电池供电，能量是宝贵资源。现有MIS算法在能量效率方面存在不足，需要设计更节能的分布式算法来延长网络寿命。

Method: 使用随机化分布式算法，在CD和no-CD两种无线电网络模型中分别设计MIS算法，通过优化节点唤醒策略来最小化能量消耗。

Result: CD模型：能量复杂度O(log n)，轮复杂度O(log² n)，失败概率1/poly(n)；no-CD模型：能量复杂度O(log²n log log n)，轮复杂度O(log³ n log Δ)，均优于现有最佳算法。

Conclusion: 成功设计了能量高效的MIS算法，在CD模型中达到最优能量复杂度，在no-CD模型中显著降低了能量消耗，为无线网络的节能运行提供了有效解决方案。
...
### [125] [Investigating Matrix Repartitioning to Address the Over- and Undersubscription Challenge for a GPU-based CFD Solver](https://arxiv.org/abs/2510.08536)
*Gregor Olenik,Marcel Koch,Hartwig Anzt*

Main category: cs.DC

TL;DR: 提出了一种重新分区策略来改进OpenFOAM中基于插件的GPU加速方法，通过更好地平衡CPU矩阵组装和GPU线性求解来缓解资源过度订阅问题。
...
Method: 提出重新分区策略，包括详细的计算模型、新颖的矩阵重新分区和更新过程，以平衡CPU矩阵组装和GPU线性求解。

Result: 该方法显著缓解了过度订阅问题，在异构CPU-GPU环境中提高了求解器性能和资源利用率。
...
### [126] [How long can you sleep? Idle Time System Inefficiencies and Opportunities](https://arxiv.org/abs/2510.07449)
*Georgia Antoniou,Haris Volos,Jawad Haj Yahya,Yiannakis Sazeides*

Main category: cs.AR

TL;DR: 提出了一个基于排队论的模型框架，用于揭示运行延迟关键型应用的现代服务器的空闲机会。通过比较实际服务器空闲时间与理论模型，发现存在大量未利用的深度空闲状态机会。


<details>
  <summary>Details</summary>
Motivation: 现代服务器在运行延迟关键型应用时存在空闲时间利用不足的问题，需要量化分析这种空闲机会以提升能效。

Method: 使用三种排队模型（M/M/1、cxM/M/1和M/M/c）来估计CPU核心和系统级别的理论空闲时间分布，并与实际服务器空闲情况进行对比分析。

Result: 实际服务器空闲时间与理论模型相比，存在显著未利用的深度空闲状态机会，主要原因是空闲管理器的准确性问题以及传统深度空闲状态转换的高延迟。

Conclusion: 该框架为早期设计探索提供了方法，能够洞察不同服务器系统配置和负载下的空闲时间行为及优化机会。
...
### [127] [DL-PIM: Improving Data Locality in Processing-in-Memory Systems](https://arxiv.org/abs/2510.07719)
*Parker Hao Tian,Zahra Yousefijamarani,Alaa Alameldeen*

Main category: cs.AR

TL;DR: DL-PIM是一种新型PIM架构，通过动态检测数据移动开销并主动将数据移动到本地内存的保留区域，使用分布式地址间接硬件查找表重定向流量，从而提高数据局部性。


<details>
  <summary>Details</summary>
Motivation: 传统PIM架构虽然能提高能效和性能，但数据移动开销会降低其优势，因为需要将数据从远程内存位置移动到处理单元进行计算。

Method: 提出DL-PIM架构，动态检测数据移动开销，主动将数据移动到请求处理单元的本地内存保留区域，使用分布式地址间接硬件查找表重定向流量，并在HMC和HBM两种3D堆叠内存上实现。

Result: DL-PIM将HMC中每个内存请求的平均延迟降低54%，HBM中降低50%；对于具有大量数据重用的工作负载，HMC性能提升15%，HBM提升5%；所有代表性工作负载在HMC中加速6%，HBM中加速3%。

Conclusion: DL-PIM通过提高数据局部性有效增强了PIM架构的整体系统性能，并通过自适应机制避免对某些工作负载产生负面影响。
...
### [128] [A Scalable FPGA Architecture With Adaptive Memory Utilization for GEMM-Based Operations](https://arxiv.org/abs/2510.08137)
*Anastasios Petropoulos,Theodore Antonakopoulos*

Main category: cs.AR

TL;DR: 提出基于FPGA的动态可配置DNN推理加速器架构，采用脉动阵列、高带宽内存和UltraRAM，支持多种处理单元配置和权重传输调度，并能模拟模拟内存计算设备。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络推理越来越依赖专用硬件以获得高计算效率，需要开发灵活可配置的加速器架构来适应不同模型和未来FPGA设计。

Method: 使用FPGA构建动态可配置加速器，包含脉动阵列、高带宽内存和UltraRAM，设计两种不同计算能力的处理单元配置，采用启发式权重传输调度策略，并能扩展模拟模拟内存计算设备。

Result: 该架构相比先前工作实现了显著的吞吐效率提升，能够模拟模拟内存计算设备以帮助下一代异构AIMC芯片设计，并能研究器件级噪声行为。

Conclusion: 本文提出了一种适用于各种模型和未来FPGA设计的通用DNN推理加速架构，具有高度适应性和可扩展性。
...
### [129] [FMCache: File-System Metadata Caching in Programmable Switches](https://arxiv.org/abs/2510.08351)
*Qingxiu Liu,Jiazhen Cai,Siyuan Sheng,Yuhui Chen,Lu Tang,Zhirong Shen,Patrick P. C. Lee*

Main category: cs.AR

TL;DR: FMCache是一个利用可编程交换机在数据平面直接服务文件系统元数据请求的框架，解决了多客户端场景下元数据缓存一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 分布式文件系统中，多台元数据服务器的快速可扩展元数据管理对于处理大量文件和目录至关重要。客户端缓存虽然能减轻服务器负载，但在客户端数量增加时会产生显著的缓存一致性维护开销和复杂性。

Method: 提出FMCache框架，利用可编程交换机直接在交换机数据平面为多个客户端提供文件系统元数据请求服务，特别解决了文件系统特定的路径依赖问题，并在严格的交换机资源约束下实现。

Result: 在Tofino交换机测试平台上使用真实文件系统元数据工作负载进行评估，FMCache相比原生HDFS实现了最高181.6%的吞吐量提升，与客户端缓存结合时还能带来额外最高139.6%的吞吐量增益，同时保持低延迟和有限的交换机资源使用。

Conclusion: FMCache通过利用可编程交换机实现了高效的文件系统元数据缓存，显著提升了分布式文件系统的性能，同时解决了传统客户端缓存面临的扩展性问题。
...
### [130] [SPAD: Specialized Prefill and Decode Hardware for Disaggregated LLM Inference](https://arxiv.org/abs/2510.08544)
*Hengrui Zhang,Pratyush Patel,August Ning,David Wentzlaff*

Main category: cs.AR

TL;DR: SPAD提出专门针对LLM推理的预填充和解码阶段设计专用芯片，通过减少硬件资源但提高利用率来降低成本。


<details>
  <summary>Details</summary>
Motivation: 当前数据中心GPU/TPU采用'越多越好'的设计理念，导致预填充阶段内存带宽利用率不足，解码阶段计算利用率不足，增加了服务成本。
...
Conclusion: SPAD设计具有长期适用性，即使模型和工作负载变化，仍能实现11%-43%的硬件成本降低。
...
### [131] [SEPhIA: <1 laser/neuron Spiking Electro-Photonic Integrated Multi-Tiled Architecture for Scalable Optical Neuromorphic Computing](https://arxiv.org/abs/2510.07427)
...
TL;DR: SEPhIA是一种光电混合的多瓦片脉冲神经网络架构，通过微环谐振器调制器和多波长光源实现每个脉冲神经元低于一个激光器的效率，在四类脉冲编码数据集上达到超过90%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 现有光学脉冲神经网络研究主要关注脉冲器件、可激发激光器网络或大型架构的数值建模，往往忽略了光功率限制、串扰和占地面积等关键约束条件。

Method: 采用微环谐振器调制器和多波长光源，通过时域协同仿真可激发CMOS-MRR耦合电路，并开发物理感知的可训练光电SNN模型，两者都使用实验获得的器件参数。

Result: 多层光电SNN在四类脉冲编码数据集上实现超过90%的分类准确率，与软件模型性能相当。设计空间研究量化了在受限信噪比条件下光子器件参数对SNN性能的影响。
...
### [132] [A Distributed Emulation Environment for In-Memory Computing Systems](https://arxiv.org/abs/2510.08257)
...
<details>
  <summary>Details</summary>
Motivation: 内存计算技术在AI设备中广泛应用，但开发此类设备并集成到系统中需要大量时间，需要实时仿真环境来在芯片可用前分析系统、测试微码和部署应用

Method: 设计了一个分布式可扩展的仿真系统架构，并开发了相应的软件开发工具

Result: 实验结果证明了所提出仿真器的实用性

Conclusion: 该仿真系统能够有效支持内存计算集成电路的快速原型设计
